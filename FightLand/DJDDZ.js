...
DJDDZ.Init=function(canvasID){
    JFunction.PreLoadData(GMain.URL).done(function () {
        ...
        GMain.BtnPanel=new JControls.Object({x:100,y:280},{width:600,height:50});//用於顯示遊戲控制按鈕
        GMain.PokerPanel0=new GControls.PokerPanel({x:100,y:5},{width:600,height:120},0,0);//用於顯示底牌，顯示對像存儲在GMain.Poker[0]
        GMain.PokerPanel1=new GControls.PokerPanel({x:200,y:355},{width:400,height:120},1,20);//用於顯示自己的牌，顯示對像存儲在GMain.Poker[1]
        GMain.PokerPanel2=new GControls.PokerPanel({x:695,y:60},{width:100,height:440},2,25);//用於顯示右邊電腦的牌，顯示對像存儲在GMain.Poker[2]
        GMain.PokerPanel3=new GControls.PokerPanel({x:5,y:60},{width:100,height:440},3,25);//用於顯示左邊電腦的牌，顯示對像存儲在GMain.Poker[3]
        GMain.PokerPanel4=new GControls.PokerPanel({x:200,y:150},{width:400,height:120},4,20);//用於顯示出的最後一手牌，顯示對像存儲在GMain.Poker[4]
...
DJDDZ.InitGame=function(){
    GMain.Poker=[];
    for(var i=0;i<5;i++)GMain.Poker[i]=[];//初始化撲克對像存儲空間
    for(var j=0;j<54;j++)GMain.Poker[0][j]=new GControls.Poker(j+1);//生成撲克對像
...
DJDDZ.SplitPoker=function(__pokerNumbers , chaiNum){//__pokerNumbers已排序，從小到大
    var splitPoker={};
    ...
    if(__pokerNumbers!=null&&__pokerNumbers.length>0){
        var _pokerNumbers=[];
        var i,j;
        for(i=0;i<__pokerNumbers.length;i++) _pokerNumbers[i]=__pokerNumbers[i];//_pokerNumbers從小到大
        ...
        for(i=chaiNum;i>=0;i--){
            ...
            for(j=_pokerNumbers.length-1;j>=i;j--){
                if(_pokerNumbers[j]==_pokerNumbers[j-i]){
                    splitPoker[str].splice(0,0,_pokerNumbers[j]);// splitPoker[str]從小到大
                    for(var k=j;k>=j-i;k--){
...
DJDDZ.IsStraight=function(numbers){//numbers已排序，從小到大
...
DJDDZ.GetPokerByType=function(__pokerNumbers,type){//從__pokerNumbers中獲取type類型的牌
    ...
    if(__pokerNumbers.length>=type.length){
        for(var i=0;i<__pokerNumbers.length;i++) _pokerNumbers[i]=__pokerNumbers[i];//_pokerNumbers從小到大
        if(type.type=="12"){//王炸
            if(_pokerNumbers[_pokerNumbers.length-1]==18&&_pokerNumbers[_pokerNumbers.length-2]==17){
                SPN.splice(0,0,18);
                SPN.splice(0,0,17);
            }
        }else if(type.type=="1"||type.type=="11"||type.type=="111"||type.type=="1111"){//非組合類型
            var c=GMain.PokerTypes[type.type].allNum-1;
            for(var j=c;j<_pokerNumbers.length;j++){//從小到大取數值
                ...
                for(var i=0;i<SPN.length;i++){
                    for(var j=0;j<_pokerNumbers.length;j++){
                        if(SPN[i]==_pokerNumbers[j]){
                            _pokerNumbers.splice(j,1);//刪除已選數值
                            break;
                        }
                    }
                }
    ...
    if(SPN.length!=type.length)SPN=[];//如果選取不成功，則清空選取數據
    return SPN;
}
...
DJDDZ.PlayPoker=function(){//出選中的牌
    ...
    GMain.LastHandNum=GMain.DealerNum;//本輪最後一手牌標識
    GMain.LastHandPokerType=DJDDZ.GetPokerType(_pokerNumbers);//設置最後一手牌牌型
    if(GMain.Poker[GMain.DealerNum].length==0){DJDDZ.GameOver();return;};//牌出完，遊戲結束
}
var GMain={
    Size:{width:800, height:480}//屏幕大小
    ,URL:""
    ,Poker:null
    ,LandlordNum:null//地主編號
    ,BeginNum:null//發牌開始編號
    ,DealerNum:null//當前操作編號
    ,MaxScore:null//搶牌最高分
    ,GrabTime:null//搶牌次數
    ,DealingHandle:null//發牌句柄
    ,DealingNum:null//已發牌數
    ,PokerSize:{width:100,height:120}//撲克牌大小
    ,LastHandNum:null//標示誰出的最後一手牌
    ,LastHandPokerType:null//最後一手牌類型
    ,ToPlay:null//已搶完地主，出牌中
    ,PokerTypes:{//撲克牌類型
...
GControls.PokerPanel=Class.create(JControls.Object,{
    ...
    ,beginShow:function($super){
        ...
        for(var i=0;i<l;i++){
            var x= 0,y= 0;
            if(this.pokerPanelNum==2||this.pokerPanelNum==3){//豎直
